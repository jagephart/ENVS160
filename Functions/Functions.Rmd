...
A function is a set of instructions, which takes an input and gives you an output. For example, a function called "add" might take two numbers as inputs and give you their sum as an output. There are some functions built into R, like "mean", which takes the mean of a set of numbers. See below that you can use the function "mean" by typing the function name, followed by parentheses. Inside the parentheses, you can indicate the "arguments". In this case, we have a vector of numbers 1, 2, 5, and 10, so the function will take the arithmatic mean: (1+2+5+10)/4

Note: we can create a vector of numbers by typing c(), as below. 
...
divide2 <- function(A, B){
  ...
  return(C) # We end the statement by telling R what the function should spit out
}
...
If you do not name the arguments like this, R assumes you just put them in the order the function wrote them in. This means that you will get a different answer from 
divide2(9, 3)
...
Functions don't only give numbers as outputs. They can also give words, evaluate logical statements as True/False, and create images. There are many functions in R for creating data visualizations. Here, we will use a basic funtion called plot, which takes the arguments "x" and "y", where x and y are vectors of numbers you want to plot as a scatterplot. The plot function also allows you to change the plot's appearance (e.g., change axes labels, change the plot symbol, change the point/line color, etc.). You can use the R help information or Google to learn more about changing these.
...
Try these making these changes and running the code chunk to view the result.
