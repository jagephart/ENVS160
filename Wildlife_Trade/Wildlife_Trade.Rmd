...
As a reminder, you can see what the data looks like by using str(CITES), which will list the columns and the data types, or head(CITES), which will show you the first 6 rows by default (you can change how many rows you view by adding n = x, where x is the number of rows you want to see). Try this below to familiarize yourself with the data sets. 
...
First, if we want to look at the total exports of all three species from 2000-2017, we will first 
...
If you want to include more than one Term, you can use "Term %in% c()", where you list the options inside c(). For example, you can replace "Term == "live"" with "Term %in% c("scales", "leather items", "teeth")".
